...
## 后记
> 近日本项目的长期赛已经展开，针对正式赛中保留的经验，同时又在网络上学习了不少新知识后有了如下结果：    
> 1. 赛题中要求提供的结果并不是直接推断是或否(即0或1)，而是需要提供具体概率，正式赛中未注意到该要求而直接Softmax导致分数偏差较大。    
> 2. NB模型与正式赛所使用的未经细致调参的XGBoost模型相比确实能取得更好的效果(0.6220)。    
> 3. 正式赛中所使用的XGBoost模型所使用的调参并不是很理想，通过一些调整后，我们很容易就在长期赛中获得了超乎想象的分数(0.7187)。     
